...
const translated = [];
const transType = ['div', 'span', 'a', 'input'];
function translateNode(node) {
  node.childNodes.forEach(child=> {
    if (translated.includes(child)) {
      return;
    }
    translated.push(child);
    if (child.nodeType !== 3) {
      if (transType.includes(child.tagName)) {
        translateNode(child);
      }
      return;
    }
    for (const [s, t] of translateDict) {
      child.nodeValue = child.nodeValue.replace(s, t);
    }
  });
}

function translateItem(observElem, selector) {
  $qsa(selector, observElem).forEach(translateNode);
}
...
async function trans(s, ts='*', d = 1) {
  const e = $qs(s);
  if (!e) {
    await pauseAsync(d);
    return await trans(s, ts, d * 2);
  }
  const observer = new MutationObserver((mutations, observer) => {
    mutations.forEach(mutation => Array.from(mutation.addedNodes).concat(mutation.target).forEach(doTrans));
  });
  doTrans(e);

  function doTrans (node) {
    if (!node instanceof Element) return;
    $qsa(ts, e).forEach(translateNode);
...
const subpageDict = {
  // persistent
  true: [
  [[0, 100, 500, 510], [101, 102, 103, 104, 203, 402, 410, 502, 503]], // 主页
    [[410]], // 市场
...
function initOpt() {
  ...
  _subpage = {
    ...
    getSubpageList: function () {
      ...
      const dict = subpageDict[query.shift()];
      ...
      for (let list of dict) {
        ...
        for (const id of ids) {
          ...
          const sub = list[1] ?? ids;
          filters = filters?.filter(s=>filterNow != s).map(s=>'&' + filterName + '='+s);
          return [_subpage.supply.includes(id), sub.filter(idx => ![sector(idx), sector(_href.alias[idx])].includes(queryJ)), filters, filtingSector];
        }
      }
    ...
    `
      ...
      const battle = v091 ? $battle.node.div : document.querySelector('.hvut-bt-div')
      if (supply && battle) {
        $id('hvut-stickyContainer').appendChild(battle);
        $battle.node.div.style.cssText += `position: sticky; z-index: 1; top: ${v091 ? 30 : 100}px; visibility: visible!important; pointer-events: all; height: 351px; background-color: #222`;
      }
    ...
    setSubpage: function (container, iframes, p, t) {
      const src = _href.get(_href.sectors[p]);
      if (!src || src.includes('undefined')) {
        return;
      }
      const elem = $element('iframe', container, {src: `${src}` + t, scrolling: 'no', style: 'width: 100%; height: 708px; border:0' });
    ...
    init: function () {
      ...
      if (!$battle.set_display_mode) {
        return;
      }
      window.addEventListener('scroll', function () {
        for (let [elem, p] of iframes) {
          if (elem.getBoundingClientRect().y > 724.49 / 2) continue;
          $battle.set_display_mode((_href.sectors[p][0] === _href.s.bt && _href.sectors[p][1] !== `iw`) ? 'fixed' : 'cancel');
          break;
        }
      });
    }
  ...
  _hath = {
    ...
    asyncCreatPriceDisplay: function () {
      console.log('asyncCreatPriceDisplay');
      // 发起 AJAX 请求以获取页面内容
...
const settings = {
  ...
  showCredits: isFrame ? undefined : 0, // 0:disable, 2:always
  showEquipSlots: isFrame ? undefined : 1, // 0:disable, 1:on battle pages only, 2:always
  trainingNotification: isFrame ? undefined : true,
  lotteryNotification: isFrame ? undefined : true,
  lotteryFilters: [
    'Rapier && Slaughter',
...
// Battle Panel: Equipment Enchant and Repair
 $battle = {
  ...
  create: function () {
    if (isFrame) {
      $battle.node.div.style.cssText+='display:none;';
       return;
    }
    $battle.load_inventory();
...
const settings = {
  ...
  */
  ...
  // [BATTLE]
  equipEnchantPosition: 'left',
  equipEnchantRepairThreshold: 20,
  equipEnchantItemInventory: {
    'Health Draught': 200,
    'Mana Draught': 200,
    'Spirit Draught': 200,
...
const $equip = {
  ...
  index: {
    category: { 'One-handed Weapon': 1, 'Two-handed Weapon': 2, 'Staff': 3, 'Shield': 4, 'Cloth Armor': 5, 'Light Armor': 6, 'Heavy Armor': 7, 'Unknown': 99 },
    type: {
      'Rapier': 1, 'Club': 2, 'Shortsword': 3, 'Axe': 4, 'Wakizashi': 5, 'Dagger': 6,
      'Estoc': 1, 'Great Mace': 2, 'Longsword': 3, 'Katana': 4, 'Scythe': 5, 'Swordchucks': 6,
      'Oak Staff': 1, 'Willow Staff': 2, 'Katalox Staff': 3, 'Redwood Staff': 4, 'Ebony Staff': 5,
      'Force Shield': 1, 'Buckler': 2, 'Kite Shield': 3, 'Tower Shield': 4,
    ...
    suffix: {
      ...
      'Protection': 31, 'Warding': 32, 'Dampening': 33, 'Stoneskin': 34, 'Deflection': 35,
      'the Fire-eater': 41, 'the Frost-born': 42, 'the Thunder-child': 43, 'the Wind-waker': 44, 'the Thrice-blessed': 45, 'the Spirit-ward': 46,
      'the Ox': 51, 'the Raccoon': 52, 'the Cheetah': 53, 'the Turtle': 54, 'the Fox': 55, 'the Owl': 56,
    },
  },
  reg: {
    name: (() => {
  ...
  parse: {
    ...
    html: function (html) {
      ...
      const info = {
        ...
        condition: parseInt(exec[8]),
        energy: exec[9] ? parseInt(exec[9]) : null,
  ...
  table: function (table, sort = true) {
    ...
    const equiplist = Array.from($qsa('tr[onmouseover*="hover_equip"]', table)).map((tr) => {
      ...
      eq.node.wrapper = tr;
      if (eq.info.customname) {
        tr.classList.add('hvut-eq-customname');
    ...
    if ($config.settings.equipSort && sort) {
      $equip.sort(equiplist, table.tBodies[0]);
    }
    return equiplist;
  },
  ...
  sort: function (equiplist, parent) {
    equiplist.sort((a, b) => {
      ...
      const k = a.info.category === 'One-handed Weapon' || a.info.category === 'Two-handed Weapon' ? ['suffix', 'quality', 'prefix']
        : a.info.category === 'Staff' ? ['prefix', 'suffix', 'quality']
          : a.info.type === 'Buckler' || a.info.type === 'Tower Shield' ? ['suffix', 'quality', 'prefix']
            : a.info.category === 'Shield' ? ['quality', 'suffix', 'prefix']
              : a.info.category === 'Cloth Armor' ? ['suffix', 'slot', 'quality', 'prefix']
                : ['slot', 'suffix', 'quality', 'prefix'];
    ...
    function create_category(text, className) {
      if (colspan) {
        ...
        $element('td', tr, { textContent: text, colSpan: colspan });
      } else {
        $element('p', frag, [text, className]);
      }
    }
    function get_colspan(table) {
      if (table.nodeName !== 'TABLE' && table.nodeName !== 'TBODY') {
        return;
      }
      const colspan = Array.from(table.rows[0].cells).reduce((s, e) => (s + parseInt(e.colSpan) || 1), 0);
      return colspan;
    }
    const colspan = get_colspan(parent);

    const frag = $element();
    const eqselall = $qs('.eqselall', parent);
    if (eqselall) {
      frag.appendChild(eqselall);
    }
    equiplist.forEach((eq, i, a) => {
      const p = a[i - 1] || { info: {} };
      if (eq.info.category !== p.info.category) {
        create_category(eq.info.category, '.hvut-eq-category');
      }
      switch (eq.info.category) {
        case 'One-handed Weapon':
        case 'Two-handed Weapon':
          if (eq.info.type !== p.info.type) {
            create_category(eq.info.type || 'Unknown', '.hvut-eq-type');
          } else if (eq.info.suffix !== p.info.suffix) {
            eq.node.wrapper.classList.add('hvut-eq-border');
          }
          break;
        ...
        case 'Shield':
          if (eq.info.type !== p.info.type) {
            create_category(eq.info.type || 'Unknown', '.hvut-eq-type');
          } else if (eq.info.suffix !== p.info.suffix && (eq.info.type === 'Buckler' || eq.info.type === 'Tower Shield')) {
            eq.node.wrapper.classList.add('hvut-eq-border');
          }
          break;
        case 'Cloth Armor':
          if (eq.info.type !== p.info.type || eq.info.suffix !== p.info.suffix) {
            create_category((eq.info.type ? `${eq.info.type} : ${eq.info.suffix || 'suffixless'}` : 'Unknown'), '.hvut-eq-type');
          } else if (eq.info.slot !== p.info.slot) {
            eq.node.wrapper.classList.add('hvut-eq-border');
          }
          break;
        ...
        case 'Heavy Armor':
          if (eq.info.type !== p.info.type || eq.info.slot !== p.info.slot) {
            create_category((eq.info.type ? `${eq.info.type} ${eq.info.slot}` : 'Unknown'), '.hvut-eq-type');
          } else if (eq.info.suffix !== p.info.suffix && (eq.info.type === 'Shade' || eq.info.type === 'Power')) {
            eq.node.wrapper.classList.add('hvut-eq-border');
          }
          break;
...
// Battle Panel: Equipment Enchant and Repair
 $battle = {
  ...
  load_dynjs: async function (doc) {
    const src = $qs('script[src*="/dynjs/"]', doc).src;
    const html = await $ajax.fetch(src + '?t=' + Date.now());
    $equip.dynjs_loaded = JSON.parse(/dynjs_equip\s?=\s?(\{.*?\});/.exec(html)?.[1] || '{}');
    $battle.equips.some((eq) => {
      const dynjs = $equip.dynjs_loaded[eq.info.eid];
      if (!dynjs) {
        $persona.change_p();
  ...
  update_condition: function () {
    $battle.equips.forEach((eq) => {
      ...
      const thld = $config.settings.equipEnchantRepairThreshold;
      const condition_span = $element('span', eq.node.condition, `${eq.info.condition}%`);
      if (eq.info.condition < thld) {
        condition_span.classList.add('hvut-bt-warn');
      }
      if (eq.info.energy !== null) {
        $element('', eq.node.condition, ' / ');
        const energy_span = $element('span', eq.node.condition, `${eq.info.energy}%`);
        if (eq.info.energy < thld) {
          energy_span.classList.add('hvut-bt-warn');
        }
      }
    });
  },
  repair: async function (eid) {
    ...
    if (buy_items.length) {
      ...
      equips.forEach((eq) => {
        eq.node.repair.innerHTML = '<span>...</span>';
  ...
  load_repair: async function (equips) {
    ...
    $battle.equips.forEach((eq) => {
      ...
      if (requires) {
        Object.entries(requires).forEach(([id, count]) => {
          const data = $battle.itemdata[id];
          $element('span', eq.node.repair, `${count} x ${data.n}`);
        });
      }
    });
  ...
  create: function () {
    ...
    equipset.forEach((info) => {
      ...
      eq.node.repair = $element('span', eq.node.li);
  ...
  init: function (outer) {
    GM_addStyle(/*css*/`
      ...
      #hvut-bt-div { visibility: hidden; position: absolute ${$id('grindfest') ? '!important' : ''}; bottom: 8px; width: 599px !important; height: 417px; margin: 0 !important; color: #333; font-size: 9pt; line-height: 20px; font-weight: normal; white-space: nowrap; }
      ...
      .hvut-bt-equip { bottom: 0; left: 0; width: 400px; height: 328px; }
      .hvut-bt-equip li { display: flex; flex-wrap: wrap; align-items: flex-start; height: 46px; border-bottom: 1px solid; }
      .hvut-bt-equip li:last-child { border-bottom: none; }
      .hvut-bt-equip li:hover { background-color: #fff; }
      .hvut-bt-equip li > a { width: 100%; overflow: hidden; text-overflow: ellipsis; border-bottom: 1px dotted #333; font-size: 10pt; line-height: 25px; font-weight: bold; text-decoration: none; }
      .hvut-bt-equip li > span:nth-child(2) { width: 100px; order: 1; cursor: pointer; }
      .hvut-bt-equip li > span:nth-child(2):hover { background-color: #fff; }
      .hvut-bt-equip li > span:nth-child(3) { visibility: hidden; flex: 1 100px; }
      .hvut-bt-equip li > span:nth-child(3) > span { margin: 5px; }
      .hvut-bt-equip li:hover > span:nth-child(3) { visibility: visible; }

      .hvut-bt-items { bottom: 0; left: 407px; width: 190px; height: 200px; }
      .hvut-bt-items li { cursor: pointer; }
      .hvut-bt-items li:hover { background-color: #fff; }
      .hvut-bt-warn { color: #e00; }

      .hvut-bt-repairall { bottom: 228px; left: 407px; width: 190px; height: 100px; cursor: pointer; }
      .hvut-bt-repairall:hover { background-color: #fff; }
    `);
...
GM_addStyle(/*css*/`
  ...
  .eqp:hover { background-color: #ddd; }
  .eqp > div:last-child { position: relative; padding: 1px 5px; line-height: 20px; white-space: nowrap; }
  div.hvut-eq-customname::after { visibility: hidden; content: attr(data-eqname); position: absolute; top: -1px; left: -1px; min-width: 100%; border: 1px solid; padding: inherit; background-color: inherit; }
  div.hvut-eq-customname:hover::after { visibility: visible; }
  p.hvut-eq-category { margin: 10px 0 5px; padding: 2px 5px; border: 1px solid; font-size: 10pt; font-weight: bold; background-color: #edb; }
  div.hvut-eq-loading p.hvut-eq-category { background-color: #eee; color: #333; }
  p.hvut-eq-type { margin: 10px 5px 5px; padding: 2px 5px; border: 1px solid; font-size: 10pt; font-weight: bold; }
  div + div.hvut-eq-border { margin-top: 11px; }
  div + div.hvut-eq-border::before { content: ''; position: absolute; margin-top: -6px; width: 100%; border-top: 1px solid #5C0D11; }
  .hvut-none-cont div.hvut-eq-border { margin-top: 0; }
  .hvut-none-cont div.hvut-eq-border::before { content: none; }

  #equiplist td { font-weight: normal; }
  #equiplist .hvut-eq-category > td { padding: 10px; border-top-width: 3px; font-size: 10pt; font-weight: bold; background-color: #edb; }
  #equiplist .hvut-eq-type > td { padding: 10px; border-top-width: 2px; font-size: 10pt; font-weight: bold; background-color: #E3E0D1; }
  #equiplist .hvut-eq-border > td { border-top-width: 3px; border-top-style: double; }
...
if ($config.settings.equipColor) {
  GM_addStyle(/*css*/`
    .eqp > div:last-child:not([onclick]) { color: #966; }
    .eqp > div:last-child[style*='color'] { box-shadow: 0 0 0 2px inset; }
    .hvut-eq-Peerless { background-color: #fbb !important; }
    .hvut-eq-Legendary { background-color: #fd8 !important; }
    .hvut-eq-Magnificent { background-color: #bdf !important; }
    .hvut-eq-Exquisite { background-color: #ce9 !important; }
    .hvut-eq-Superior { background-color: #ccc !important; }
  `);
}
...
_top.menu = {
  ...
  //'The Armory': { s: 'Bazaar', ss: 'am', button: 'AM', text: 'The Armory' },
  'Organize': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'organize', button: 'OR', text: 'Organize' },
  'Modify': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'modify', button: 'MO', text: 'Modify' },
  'Repair': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'repair', button: 'RE', text: 'Repair' },
  'Soulbind': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'soulbind', button: 'SB', text: 'Soulbind' },
  'Purchase': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'purchase', button: 'PU', text: 'Purchase' },
  'Sell': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'sell', button: 'SE', text: 'Sell' },
  'Salvage': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'salvage', button: 'SA', text: 'Salvage' },
...
const $dfct = {
  ...
  create: function () {
    ...
    $dfct.sub = $element('div', $dfct.div, ['.hvut-top-sub']);
    const options = $dfct.list.map((d, i) => `${i + 1}:${d}`).reverse();
    $dfct.selector = $input(['select', options], $dfct.sub, { size: $dfct.list.length, className: 'hvut-scrollbar-none', style: 'width: 80px;' }, {
      change: () => {
        $dfct.selector.disabled = true;
        $dfct.change($dfct.selector.value);
      }
...
const $persona = {
  ...
  create: function () {
    ...
    $persona.selector_p = $input('select', $persona.sub, { size: json.plen, className: 'hvut-scrollbar-none', style: 'width: 110px;' }, {
      change: () => {
        $persona.selector_p.disabled = true;
        $persona.change_p($persona.selector_p.value);
      }
    ...
    $persona.selector_e = $input('select', $persona.sub, { size: json.elen, className: 'hvut-scrollbar-none', style: 'width: 110px;' }, {
      change: () => {
        $persona.selector_e.disabled = true;
        $persona.change_e($persona.selector_e.value);
      }
...
if ($config.settings.showEquipSlots === 2 || $config.settings.showEquipSlots === 1 && _query.s === 'Battle') {
  _bottom.show_equip = async function () {
    ...
    const exec = /<td>Inventory Capacity:<\/td><td>(\d+)<\/td><td>\/<\/td><td>(\d+)<\/td>/.exec(html);
    const usage = parseInt(exec[1]);
    const capacity = parseInt(exec[2]);
...
if (_query.s === 'Character' && _query.ss === 'ch' || $id('persona_outer')) {
  ...
  if (_query.s === 'Character' && _query.ss === 'eq') {
    _eq.mage_stats = function () {
      // to get exact numbers
      const stats_pane = _eq.stats_pane;
      const stats_equip = $equip.parse.equipstats(_eq.equiplist);
    ...
    if (_query.equip_slot) {
      $equip.table($qs('#equiplist > table'));
    } else {
      GM_addStyle(/*css*/`
      ...
      .hvut-eq-buttons { display: flex; width: 650px; margin: 5px auto; text-align: left; }
      .hvut-eq-info { position: absolute; top: 0; right: 5px; font-size: 9pt; font-weight: normal; }
      .hvut-eq-info > span { display: inline-block; }
      .hvut-eq-info > span:nth-child(2) { width: 35px; }
      .hvut-eq-info > span:nth-child(3) { width: 35px; }
      .hvut-eq-info > span:nth-child(4) { width: 50px; }
      .hvut-eq-untradeable { color: #c00; }
      .hvut-eq-warn { color: #c00; }
    ...
    `);
      ...
      _eq.equiplist.forEach((eq) => {
        $element('div', eq.node.wrapper.firstElementChild, ['.hvut-eq-info']).append(
          $element('span', null, [(eq.info.soulbound ? 'Soulbound' : 'Lv.' + eq.info.level), (!eq.info.tradeable ? '.hvut-eq-untradeable' : '')]), ' : ',
          $element('span', null, [`${eq.info.condition}%`, (eq.info.condition <= 20 ? '.hvut-eq-warn' : '')]), ' / ',
          $element('span', null, [eq.info.energy ? `${eq.info.energy}%` : '-', (eq.info.energy <= 20 ? '.hvut-eq-warn' : '')]), ' : ',
          $element('span', null, eq.info.soulbound ? `${eq.info.upgrade} / ${eq.info.iw}` : '-')
        );
      });
  ...
  `);
                    ...
                    $ajax.fetch('?s=Bazaar&ss=am&screen=organize').then((html) => {
                      const exec = /<td>Inventory Capacity:<\/td><td>(\d+)<\/td><td>\/<\/td><td>(\d+)<\/td>/.exec(html);
                      _ss.equip.usage = parseInt(exec[1]);
                      _ss.equip.capacity = parseInt(exec[2]);
  ...
  `);
    ...
    `);
                            ...
                            _mm.db = {
                              ...
                              import: function () {
                                _mm.node.db_import.disabled = true;
                                const input = $input('file', null, { accept: '.json' }, {
                                  change: () => {
                                    const file = input.files[0];
                                    if (!file) {
                                      return;
    ...
    `);
                          ...
                          if (_query.s === 'Bazaar' && (_query.ss === 'lt' || _query.ss === 'la')) {
                            ...
                            if (_query.s === 'Battle') {
                              GM_addStyle(/*css*/`
    #arena_list { white-space: nowrap; }
    #arena_list th:nth-child(1) { width: 474px; }
    #arena_list th:nth-child(2) { width: 120px; }
    #arena_list th:nth-child(3) { width: 90px; }
    #arena_list th:nth-child(4) { width: 90px; }
    #arena_list th:nth-child(5) { width: 90px; }
    #arena_list th:nth-child(6) { width: 90px; }
    #arena_list th:nth-child(7) { width: 120px; }
    #arena_list th:nth-child(8) { width: 90px; }
                                ...
                                if (_query.ss === 'rb') {
                                  ...
                                  if (_query.ss === 'tw') {
                                    ...
                                    if (_query.ss === 'gr') {
                                      ...
                                      // [END 19] Battle - GrindFest */


                                      //* [20] Battle - Item World
                                      if (_query.ss === 'iw') {
                                        $equip.table($qs('#equiplist > table'));
...
if (_query.s === 'Bazaar' && _query.ss === 'am' && $id('equiplist')) {
  ...
  if (_query.screen === 'purchase' || _query.screen === 'sell' || _query.screen === 'salvage') {
    function parse_dynjs() {
      const script = $qs('#equipform ~ script:last-child');
      if (script) {
        const html = script.innerHTML;
        Object.assign($equip.dynjs_eqstore, JSON.parse(/dynjs_eqstore\s?=\s?(\{.*?\});/.exec(html)?.[1] || null));
        Object.assign($armory.eqitems, JSON.parse(/eqitems\s?=\s?(\{.*?\});/.exec(html)?.[1] || null));
        Object.assign($armory.itemdata, JSON.parse(/itemdata\s?=\s?(\{.*?\});/.exec(html)?.[1] || null));
      }
    }
    parse_dynjs();
  }
